document.addEventListener('DOMContentLoaded', () => {
  ...
  // Configura botão Sair no canto direito do cabeçalho e limpa status bar indesejada
  try {
    const header = document.querySelector('header.site-header') || document.querySelector('header');
    if (header) {
      // Limpa barras de status e botões duplicados/antigos
      header.querySelectorAll('#logoutBtn, #adminBadgeExit, #btnLogoutHeader, #btnLogoutInline, #btnExitNearStatus, #btnExitRight, #logout, .header-logout, .admin-badge-exit, .btn-exit-admin, #clienteStatus, #statusLogado, .status-admin, .header-user').forEach(el => el.remove());

      // Conecta o botão Sair oficial à direita
      let btnExit = document.getElementById('btnExitHeader');
      if (!btnExit) {
        btnExit = document.createElement('button');
        btnExit.id = 'btnExitHeader';
        btnExit.className = 'btn-header-exit';
        btnExit.type = 'button';
        btnExit.title = 'Sair do modo funcionário';
        btnExit.innerHTML = '<i class="fa-solid fa-right-from-bracket" aria-hidden="true"></i><span>Sair</span>';
        header.appendChild(btnExit);
      }
      btnExit.style.display = 'inline-flex';
      btnExit.onclick = logoutFuncionario;
    }
  } catch (e) { /* ignore */ }
